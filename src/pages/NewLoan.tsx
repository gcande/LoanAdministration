...
import type { PlanPrestamo } from "../services/types";

const NewLoan = () => {
  const navigate = useNavigate();
  const [clientes, setClientes] = useState<
    { id: string; nombre: string; identificacion: string }[]
  >([]);
  const [planes, setPlanes] = useState<PlanPrestamo[]>([]);
  ...
  const [selectedPlan, setSelectedPlan] = useState<PlanPrestamo | null>(null);
  ...
  const fetchConfig = async () => {
    const valor = await fetchConfigByClave("sistema_amortizacion");
    if (valor) setAmortSystem(valor as "frances" | "flat");
  };
  ...
  const handleCreateLoan = async () => {
    ...
    try {
      const { error } = await createPrestamoConCuotas({
        prestamo: {
        ...
        <div className="form-grid">
            ...
            >
              ...
              {planes.map((p) => (
                <option key={p.id} value={p.id}>
                  {p.nombre_plan} - {p.num_cuotas} {p.frecuencia_pago}s -{" "}
                  {p.tasa_interes}%
                </option>
              ))}
            </select>
          ...
          {selectedPlan && (
            <>
              <div className="form-group">
                <label>
                  Monto ({formatCurrency(selectedPlan.monto_minimo)} -{" "}
                  {formatCurrency(selectedPlan.monto_maximo)})
                </label>
                <input
                  type="number"
          ...
          <div className="loan-summary-grid">
            ...
            <div className="summary-card">
              ...
              <div className="summary-details">
                ...
                <strong>
                  {formatCurrency(
                    schedule.reduce((acc, row) => acc + row.monto_cuota, 0),
                  )}
                </strong>
              </div>
            </div>
            ...
            <div className="summary-card">
              ...
              <div className="summary-details">
                ...
                <strong>
                  {formatCurrency(
                    schedule.reduce((acc, row) => acc + row.monto_interes, 0),
                  )}
                </strong>
              </div>
            </div>
