...
const Clients = () => {
  ...
  const fetchClientesList = async () => {
    setLoading(true);
    try {
      const { data, count } = await fetchClientes(
        { page, pageSize },
        searchTerm,
      );
  ...
  const fetchClientDetails = async (cliente: Cliente) => {
    ...
    try {
      ...
      const graceValor = await fetchConfigByClave("dias_gracia");
      const graceDays = Number(graceValor || 0);

      // Fetch loans + cuotas
      const { prestamos: loans, cuotas: installments } =
        await fetchClienteDetalle(cliente.id);

      // Calculate stats
      const totalBorrowed =
        loans?.reduce((sum, l) => sum + Number(l.monto_prestado), 0) || 0;
      const activeLoans =
        loans?.filter((l) =>
          ["activo", "al_dia", "en_mora"].includes(l.estado),
        ) || [];
      ...
      const punctuality = punctualityHasHistory
        ? calculatePunctuality(installments)
        : 0;
      ...
      const lateInstallmentsCount = countLateInstallments(
        installments,
        todayIso,
        graceDays,
      );

      const balance = activeLoans.reduce(
        (sum, l) => sum + Number(l.saldo_pendiente),
        0,
      );
      const balanceRatio =
        totalBorrowed > 0 ? balance / totalBorrowed : balance > 0 ? 1 : 0;

      let riskScore = 100;
      if (!punctualityHasHistory) riskScore -= 20;
      riskScore -= Math.min(lateInstallmentsCount * 12, 36);
      if (punctualityHasHistory)
        riskScore -= Math.max(0, 85 - punctuality) * 0.7;
      riskScore -= Math.min(avgDelayDays * 2, 20);
      if (balanceRatio > 0.6) riskScore -= 15;
      else if (balanceRatio > 0.35) riskScore -= 8;
      if (totalLoans >= 8 && punctuality >= 90 && lateInstallmentsCount === 0)
        riskScore += 8;
      riskScore = Math.round(clamp(riskScore, 0, 100));

      const riskLevel: "Bajo" | "Medio" | "Alto" =
        riskScore >= 75 ? "Bajo" : riskScore >= 50 ? "Medio" : "Alto";

      const averageLoanAmount =
        totalLoans > 0 ? totalBorrowed / totalLoans : 500000;
      const riskMultiplier =
        riskLevel === "Bajo" ? 1.25 : riskLevel === "Medio" ? 0.9 : 0.6;
      const punctualityMultiplier = punctualityHasHistory
        ? 0.8 + (punctuality / 100) * 0.3
        : 0.75;
      const delinquencyMultiplier = Math.max(
        0.6,
        1 - lateInstallmentsCount * 0.08,
      );
      const suggestedMaxAmount = Math.round(
        clamp(
          averageLoanAmount *
            riskMultiplier *
            punctualityMultiplier *
            delinquencyMultiplier,
          250000,
      ...
      if (!punctualityHasHistory)
        riskFactors.push(
          "Sin historial de cuotas pagadas para medir puntualidad.",
        );
      else
        riskFactors.push(
          `Puntualidad historica: ${punctuality}% (${onTimeInstallmentsCount}/${paidInstallments.length}).`,
        );
      if (lateInstallmentsCount > 0)
        riskFactors.push(
          `Tiene ${lateInstallmentsCount} cuota(s) en mora activa.`,
        );
      if (maxDelayDays > 0)
        riskFactors.push(
          `Atraso promedio ${avgDelayDays} dia(s), maximo ${maxDelayDays} dia(s).`,
        );
      if (balanceRatio > 0.35)
        riskFactors.push(
          `Saldo activo elevado (${Math.round(balanceRatio * 100)}% del total prestado).`,
        );
      if (totalLoans >= 6 && lateInstallmentsCount === 0)
        riskFactors.push("Historial amplio sin mora activa reciente.");
      if (riskFactors.length === 0)
        riskFactors.push(
          "Comportamiento estable en los indicadores evaluados.",
        );
  ...
  const handleCreateClient = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const { data, error } = await createCliente(newClient);
  ...
  const handleUpdateClient = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await updateCliente(editingClient.id, {
      ...
      if (data) {
        setClientes(
          clientes.map((c) => (c.id === editingClient.id ? data : c)),
        );
      }
  ...
  const handleDeleteClient = async (id: string) => {
    if (window.confirm("¿Estás seguro de que quieres eliminar este cliente?")) {
      try {
        const { error } = await softDeleteCliente(id);
        if (error) throw error;
    ...
    <Layout title="Clientes">
      <div className="clients-toolbar card animate-fade">
        <div className="search-bar clients-search-bar">
          ...
          <button
            className="btn-filter clients-filter-btn"
        ...
        <div className="actions-header clients-actions-header">
          {profile?.rol === "admin" && (
            <button
              className="btn btn-primary clients-new-btn"
          ...
          <p className="clients-count">{totalCount} clientes registrados</p>
        </div>
      </div>
            ...
            {clientes.map((cliente) => (
              <div key={cliente.id} className="card client-card">
                <div className="client-main-content">
                  ...
                  <div className="client-info">
                    ...
                    <div className="client-quick-actions">
                      <span>
                        <Phone size={14} /> {cliente.telefono || "Sin telefono"}
                      </span>
                      <span>
                        <Mail size={14} /> {cliente.email || "Sin email"}
                      </span>
                    </div>
                  </div>
                </div>
                <div className="client-status">
                  <span
                    className={`badge ${cliente.estado === "activo" ? "badge-success" : "badge-danger"}`}
                  >
                    {cliente.estado}
                  </span>
                  ...
                  <div className="client-details-shell animate-fade">
                    <div className="client-details-hero">
                      ...
                      <div className="client-hero-info">
                        ...
                        <div className="client-hero-badges">
                          <span
                            className={`badge ${selectedClient.estado === "activo" ? "badge-success" : "badge-danger"}`}
                          >
                            {selectedClient.estado}
                          </span>
                          {clientStats &&
                            clientStats.punctualityHasHistory &&
                            clientStats.punctuality >= 90 &&
                            clientStats.lateInstallmentsCount === 0 && (
                    ...
                    <div className="client-kpi-grid">
                      <div className="client-kpi-card">
                        <span className="client-kpi-label">
                          <DollarSign size={14} /> Total prestado
                        </span>
                        <span className="client-kpi-value">
                          ${clientStats?.totalBorrowed.toLocaleString()}
                        </span>
                      </div>
                      <div className="client-kpi-card">
                        <span className="client-kpi-label">
                          <Clock size={14} /> Saldo actual
                        </span>
                        <span className="client-kpi-value text-primary">
                          ${clientStats?.balance.toLocaleString()}
                        </span>
                      </div>
                      <div className="client-kpi-card">
                        <span className="client-kpi-label">
                          <CheckCircle size={14} /> Puntualidad
                        </span>
                        <span
                          className={`client-kpi-value ${clientStats?.punctualityHasHistory ? "text-success" : "text-secondary"}`}
                        >
                          {clientStats?.punctualityHasHistory
                            ? `${clientStats?.punctuality}%`
                            : "N/A"}
                        </span>
                      ...
                      <div className="client-kpi-card">
                        <span className="client-kpi-label">
                          <AlertCircle size={14} /> Moras
                        </span>
                        <span className="client-kpi-value text-danger">
                          {clientStats?.lateInstallmentsCount} cuotas
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="client-insight-card">
                      ...
                      <div className="client-insight-row">
                        ...
                        <strong
                          className={
                            clientStats?.riskLevel === "Bajo"
                              ? "text-success"
                              : clientStats?.riskLevel === "Medio"
                                ? "text-warning"
                                : "text-danger"
                          }
                        >
                          {clientStats?.riskLevel} ({clientStats?.riskScore}
                          /100)
                        </strong>
                      </div>
                      <div className="client-insight-row">
                        ...
                        <strong>
                          $
                          {(
                            clientStats?.suggestedMaxAmount || 0
                      ...
                      <div className="client-insight-row">
                        ...
                        <strong>
                          {clientStats?.maxDelayDays
                            ? `${clientStats?.avgDelayDays}d prom / ${clientStats?.maxDelayDays}d max`
                      ...
                      <div className="client-insight-row">
                        <span>Historial</span>
                        <strong>
                          {clientStats?.totalLoans} prestamos otorgados
                        </strong>
                      </div>
                    ...
                    <div className="client-loans-card">
                      ...
                      <div className="loans-tiny-list">
                        {clientStats?.loans.length === 0 ? (
                          <p className="client-loans-empty">
                            Sin historial de prestamos
                          </p>
                        ) : (
                          clientStats?.loans.map((loan) => (
                            <div key={loan.id} className="loan-tiny-card">
                              <div className="loan-tiny-info">
                                <span className="loan-tiny-plan">
                                  {loan.planes_prestamo?.nombre_plan ||
                                    "Plan sin nombre"}
                                </span>
                                <span className="loan-tiny-date">
                                  {new Date(
                                    loan.created_at,
                                  ).toLocaleDateString()}
                                </span>
                              ...
                              <div className="loan-tiny-amount">
                                <span className="amount">
                                  ${loan.monto_prestado.toLocaleString()}
                                </span>
                                <span
                                  className={`badge-dot ${loan.estado}`}
                                  title={loan.estado}
                                ></span>
                              </div>
                            </div>
                    ...
                    <div className="modal-actions client-details-actions">
                      <button
                        className="btn btn-primary w-full client-close-btn"
